...
describe('F086 M2: Meta-thinking trigger documentation', () => {
  const sharedRules = readFileSync(
    resolve(__dirname, '../../../cat-cafe-skills/refs/shared-rules.md'),
    'utf-8',
  );
  ...
  it('triggerType enum values match documented triggers', () => {
    ...
    const callbackToolsSrc = readFileSync(
      resolve(__dirname, '../../mcp-server/src/tools/callback-tools.ts'),
      'utf-8',
    );
    const enumMatch = callbackToolsSrc.match(/z\.enum\(\[([^\]]+)\]\).*triggerType|triggerType.*z\.enum\(\[([^\]]+)\]\)/);
    assert.ok(enumMatch, 'Could not find triggerType z.enum in callback-tools.ts');
    const rawValues = (enumMatch[1] || enumMatch[2]).replace(/'/g, '').split(',').map(s => s.trim());
  ...
  it('feat-lifecycle contains Design Gate 先搜现状 check', () => {
    const featLifecycle = readFileSync(
      resolve(__dirname, '../../../cat-cafe-skills/feat-lifecycle/SKILL.md'),
      'utf-8',
    );
