...
describe('Signal-to-Noise: SQLite vs grep', () => {
	...
	it('SQLite returns fewer, more relevant results than grep for "adapter"', async () => {
		...
		const grepOutput = execSync(
			`grep -rl "adapter" "${docsRoot}" 2>/dev/null || true`,
			{ encoding: 'utf-8' },
		);
		...
		const noiseFiles = grepFiles.filter(f =>
			f.includes('/archive/') || f.includes('/mailbox/') || f.includes('/stories/') || f.includes('/research/'),
		);
	...
	it('SQLite excludes archive/mailbox content by design', async () => {
		...
		for (const r of allResults) {
			if (r.sourcePath) {
				...
				assert.ok(
					isAllowed,
					`Result sourcePath "${r.sourcePath}" is outside allowed directories`,
				);
			}
		}
	});
